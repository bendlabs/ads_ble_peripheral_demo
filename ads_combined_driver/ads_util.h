...
#define ADS_AXIS_0_EN		(0x01)			// axis 0 enable bit, two axis only
#define ADS_AXIS_1_EN		(0x02)			// axis 1 enable bit, two axis only 
...
typedef enum {
	...
	ADS_READ_STRETCH,		// Enable simultaneous bend and stretch measurements One Axis Only
	ADS_AXES_ENALBED = 8,	// Enable/Disable measurements on one or both axes. Two Axis Only
...
/* Device IDS */
typedef enum {
	ADS_ONE_AXIS   = 1,		// Device ID for one axis sensor
	ADS_TWO_AXIS   = 2,		// Device ID for two axis sensor
	ADS_TWO_REGION = 3
} ADS_DEV_IDS_T;
